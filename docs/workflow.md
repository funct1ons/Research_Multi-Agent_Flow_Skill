...
This workflow coordinates Codex and one or more execution agents in a personal research repository without letting any agent become the project owner.
...
- The execution agent writes a handoff note.
...
4. Give the task card to an execution agent for implementation.
5. Require the execution agent to update the handoff section.
6. Inspect `git status` and `git diff`.
7. Ask Codex for read-only review.
8. Send bounded follow-up work back to the execution agent if needed.
...
Only the execution agent should move `in_progress` to `implemented`, and only after filling handoff. Only review can move a task to `ready_to_merge`. Only the human user can move a task to `done`.

## Execution Agent Options

The execution agent is a role, not a fixed tool. It may be:

- OpenCode
- Reasonix
- a low-cost Codex subagent
- another local coding agent that can read task cards, edit files, run validation commands, and write handoff notes

Codex remains the default planner and reviewer. If a Codex subagent implements a task, the reviewing Codex instance must treat the task card, handoff, validation output, and `git diff` as the source of truth.
...
An execution agent implements the task and fills the handoff. The human checks the diff. Codex reviews whether only dropout changed, whether seeds are explicit, and whether a multi-seed follow-up is needed. The human decides whether to merge.
