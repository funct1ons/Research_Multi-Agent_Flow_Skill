...
这份文档面向工作流的使用者，帮助你理解如何在个人科研项目中同时使用 Codex、OpenCode、Git、任务卡和实验记录。
...
OpenCode 不应该自由决定实验设计，也不应该随意修改 baseline 语义。
...
- 任何需要 OpenCode 实现、Codex review 的任务。
...
```text
请根据当前研究目标，把今天的工作拆成 2 个适合 OpenCode 执行的小任务。
每个任务需要包含：
- Goal
- Background
- Allowed files
...
- Codex 可以 review，但不要和 OpenCode 同时在同一分支上改代码。

### Step 4：OpenCode 执行任务卡

给 OpenCode 的指令应该非常明确：
...
如果发现明显越界，先让 OpenCode 解释或回退，不要进入 review。
...
OpenCode 实现后填写 handoff。你检查 diff，确认没有改 `src/model.py`、`src/data.py`、`scripts/run_train.py` 或 `paper/`。Codex review 通过后，你提交并 squash merge。正式跑实验后，再把 config、command、commit、metrics 和 notes 放进 `experiments/`。
...
review 默认只读。发现问题后，优先生成 follow-up task card，再交给 OpenCode 实现。
...
### 错误 5：把 OpenCode 当成研究决策者

OpenCode 适合执行任务卡，不适合判断实验是否支撑论文 claim。研究判断必须由你负责，Codex 可以辅助审查。
