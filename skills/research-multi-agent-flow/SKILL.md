...
description: Use when managing personal research codebases with Codex, OpenCode, Git branches/worktrees, task cards, handoff notes, experiment records, or multi-agent coding/review workflows.
---
...
Use this skill to keep personal research projects reproducible while coordinating Codex and OpenCode through files and Git. The core rule is: stable policy belongs in `AGENTS.md`, concrete work belongs in `tasks/*.md`, implementation handoff belongs in the task card, research facts belong in `experiments/`, and the human user owns final research and merge decisions.
...
| OpenCode | Local implementation, tests, config/script edits, lint/type/test repair loops, handoff notes |
...
- Read `references/review-prompts.md` when the user asks for Codex/OpenCode handoff, diff review, or follow-up task prompts.
